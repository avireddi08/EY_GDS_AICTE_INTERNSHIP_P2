...
function AuctionItem() {
  ...
  const handleBid = async () => {
    ...
    try {
      const res = await axios.post(`http://localhost:5001/bid/${id}`, { bid: bidValue, username });
      ...
      setMessage(`Error in placing a bid: ${error.response?.data?.message || error.message}`);
    }
  };
